...
class Main:
        ...
        if self.context == Context.ON_DELETE_USER:
            ...
            if type(response_1) == it:  # handle info type responses
                ...
                if data == 'Y':  # if admin wishes to proceed verify his or her legitimacy
                    # create a verifier object with recipient set to email of admin and
                    ...
                    msg = verifier.generate_message(to=admin.logged_in_admin.email,
                                                    username=admin.logged_in_admin.username, token=token)
    ...
    def _get_input_data(self):
        ...
        if self.context in (Context.ON_LOGIN_ADMIN, Context.ON_LOGIN_USER):
            ...
            while True:
                email = input("Email: ")
                if not email.isspace() or not email:
                    data.append(email)
                    break
            ...
            if self.context == Context.ON_SIGN_UP_ADMIN:
                ...
                while True:
                    ...
                    try:
                        ...
                        print(
                            "👩‍💻 Hmm, Please enter a digit.❌ Can you not see👀 from the available options 👆 above?")
                    else:
                        data.append(type_)
                        break
            ...
            while True:
                ...
                if self.prompt == '1':  # if option is 1 which is home,
                    ...
                    if type(response) == it:
                        ...
                        self.rich.print_home_data(data,
                                                  home_logos[random.randint(0, 4)])  # finally the data print it out
                    ...
                    # handle other responses apart from the data
                    if type(response) == it:
                        self.handle_response(response)
                    else:
                        data = response
                        ...
                        self.rich.my_profile_display(data=data,
                                                     progress=self.current_user.profile_strength(),
                                                     panel_template=profile_panel_user)
