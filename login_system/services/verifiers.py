...
class Verify():
    ...
    def _get_body_html(self):
        ...
        bodies = (
        """\
        <!DOCTYPE html>from enum import Enum, unique
from email.message import EmailMessage
import smtplib, socket
        ...
        try:
            with self.console.status("[hot_pink]Getting ready to send message!", spinner='runner') as status:
                ...
                with smtplib.SMTP_SSL('smtp.gmail.com') as smtp: # ssl smtp
                    ...
                    self.console.log("[cyan2] Authentication successful 👊✅[/cyan2]")
                    status.update(
                        status="💌💨 [orange3] Sending Message. [/orange3]",
                        spinner="pong",
                        spinner_style="yellow"
                    )
                    sleep(2.50)
                    smtp.send_message(msg) # try to send the message
                    self.console.log("[light_green blink] 💌 Message sent succesfully.✅[/light_green blink]")
